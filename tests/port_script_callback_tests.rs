...
async fn test_create_script_engine_invalid_lua() {
    ...
    assert!(
        result.is_err(),
        "Should fail to create engine from invalid Lua"
    );
}
...
async fn test_on_send_echo() {
    ...
    assert_eq!(
        output, test_data,
        "Echo script should return data unchanged"
    );
}
...
async fn test_on_recv_with_prefix() {
    ...
    let output = engine.on_recv(test_data);
    assert_eq!(
        output, b"IN:response",
        "Prefix script should add IN: prefix"
    );
}
...
async fn test_on_recv_error() {
    ...
    assert_eq!(
        output, test_data,
        "Error in on_recv should pass through data"
    );
}
...
async fn test_intercept_on_send() {
    ...
    assert_eq!(
        output,
        Vec::<u8>::new(),
        "Intercept should return empty vec"
    );
}
...
async fn test_empty_data_handling() {
    ...
    assert_eq!(
        output_recv, empty_data,
        "Should handle empty data in on_recv"
    );
}
...
async fn test_multiple_engines_independent() {
    ...
    assert_eq!(
        result2.unwrap(),
        b"OUT:test",
        "Prefix engine should add prefix"
    );
}
...
async fn test_on_recv_without_function() {
    ...
    assert_eq!(
        output, test_data,
        "Should pass through data when on_recv missing"
    );
}
...
async fn test_script_returning_nil() {
    ...
    assert_eq!(
        result.unwrap(),
        Vec::<u8>::new(),
        "Nil should result in empty vec"
    );
}
